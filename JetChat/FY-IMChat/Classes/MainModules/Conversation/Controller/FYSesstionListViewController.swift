...
extension FYSesstionListViewController: FYPopListMenuDelegate {
    ...
    func showPicker() {
        let pickerView = FYDatePickerView()
        pickerView.isScreenClose = true
        pickerView.show()
        pickerView.dateSelectedClosure = { [weak self] date in
            self?.navigationItem.title = date
        }
    }
    
    func webBrowser() {
        let webVC = WkWebBrowserViewController(url: "https://www.baidu.com/")
